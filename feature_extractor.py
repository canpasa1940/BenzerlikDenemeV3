...
def log_attack_features(env, sr, hop):
    """Attack time ve slope hesaplama"""
    if env.max() < EPS:
        return 0.0, 0.0
...
def hpss_energy_ratio(y):
    """Harmonic-Percussive ayrımı enerji oranı"""
    y_h, y_p = librosa.effects.hpss(y)
    return np.sum(y_p**2) / (np.sum(y_h**2) + EPS)

def extract_features(signal, sr=SR):
    """
    Ses sinyalinden özellik çıkarma
    Notebook'taki extract_features fonksiyonu ile aynı
    """
    if len(signal) < N_FFT:
...
def extract_features_from_file(file_path):
    """
    Dosya yolundan özellik çıkarma
    """
    try:
